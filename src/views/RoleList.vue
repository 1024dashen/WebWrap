...
const handleMenuChange = (menu: Permission, checked: boolean) => {
    ...
    if (checked) {
        ...
        selectedPermissions.value = selectedPermissions.value.filter(
            (code) =>
                code !== menu.code && !buttons.some((b) => b.code === code),
        )
    }
}
            ...
            <div class="permission-tree">
                ...
                >
                    <div class="permission-menu-row">
                        ...
                        <el-checkbox
                            :model-value="
                                selectedPermissions.includes(menu.code)
                            "
                            @change="
                                (val: boolean) => handleMenuChange(menu, val)
                            "
                        >
                    ...
                    <div
                        ...
                        class="permission-button-row"
                    >
                        <el-checkbox
                            v-for="btn in getButtonPermissions(menu.id)"
                            :key="btn.id"
                            :model-value="
                                selectedPermissions.includes(btn.code)
                            "
                            @change="
                                (val: boolean) =>
                                    handleButtonChange(menu, btn.code, val)
                            "
                        >
                            {{ btn.name }}
                            <el-tag size="small" type="info">{{
